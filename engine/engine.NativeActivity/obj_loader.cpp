...
namespace tinyobj {
	...
	std::string LoadMtl(std::map<std::string, int> &material_map,
		...
		for (int _inx = 0; _inx < data.lenght; ++_inx){
			char *line_begin = buf + _inx;
			while (!isNewLine(buf[_inx]) && _inx < data.lenght)
				++_inx;
			buf[_inx] = 0;

			std::string linebuf(line_begin);
	...
	std::string MaterialFileReader::operator()(const std::string &matId,
		...
		bytes data = Asset::read(filepath);
		return LoadMtl(matMap, materials, data);
	}

	void mesh_t::logi() {

		LOGI("Size of indices: %i\n",(int)this->indices.size());
		LOGI("Size of material_ids: %i\n",(int)this->material_ids.size());
		assert((this->indices.size() % 3) == 0);
		for (size_t f = 0; f < this->indices.size() / 3; f++) {
			LOGI("  idx[%i] = %d, %d, %d. mat_id = %d\n", f, (int)this->indices[3 * f + 0], this->indices[3 * f + 1],this->indices[3*f+2], this->material_ids[f]);
		}

		LOGI("vertices: %d\n", this->positions.size());
		assert((this->positions.size() % 3) == 0);
		for (size_t v = 0; v < this->positions.size() / 3; v++) {
			LOGI("  v[%d] = (%f, %f, %f)\n", (int)v,
	...
	std::string LoadObj(std::vector<shape_t> &shapes,
		...
		bytes data = Asset::read(filename);
		...
		return LoadObj(shapes, materials, data, matFileReader);
	}

	std::string LoadObj(std::vector<shape_t> &shapes,
						std::vector<material_t> &materials, // [output]
						bytes data, MaterialReader &readMatFn) {
		std::stringstream err;
